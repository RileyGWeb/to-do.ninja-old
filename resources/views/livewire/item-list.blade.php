<div>
    @foreach($items as $item)    
        <div class="flex" :wire:key="{{ $item->id }}">
            <input id="item_complete" onChange="this.form.submit()" type="checkbox" wire:ignore {{ ($item->completed) ? "checked" : "" }} wire:model="completed.{{ $item->id }}" class="w-4 h-4 text-blue-600 bg-white rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
